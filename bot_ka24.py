...
sys.stdout.flush()
print("🚀 НАЧАЛО ЗАГРУЗКИ БОТА", flush=True)
...
ADMIN_VK_ID = int(os.getenv("ADMIN_VK_ID")) if os.getenv("ADMIN_VK_ID") else None

print(f"🔍 VK_TOKEN: {'✅ есть' if VK_TOKEN else '❌ нет'}", flush=True)
print(f"🔍 GROUP_ID: {GROUP_ID}", flush=True)
print(f"🔍 ADMIN_VK_ID: {ADMIN_VK_ID}", flush=True)

if not VK_TOKEN or not GROUP_ID or not ADMIN_VK_ID:
    print("❌ ОШИБКА: Не все переменные окружения загружены!", flush=True)
    sys.exit(1)

print("✅ Токены успешно загружены", flush=True)
...
def get_google_creds():
    ...
    print("🔐 Загружаем Google Credentials...", flush=True)
    
    creds_json_str = os.getenv("GOOGLE_CREDENTIALS_JSON")
    
    if not creds_json_str:
        print("❌ GOOGLE_CREDENTIALS_JSON не найдена!", flush=True)
        raise Exception("Нет учётных данных Google")
    
    try:
        creds_dict = json.loads(creds_json_str)
        print("✅ JSON успешно распарсен", flush=True)
        
        if 'private_key' in creds_dict:
            print(f"✅ private_key найден", flush=True)
            creds_dict['private_key'] = creds_dict['private_key'].replace('\\n', '\n')
        
        scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
        creds = ServiceAccountCredentials.from_json_keyfile_dict(creds_dict, scope)
        print("✅ Учётные данные Google успешно созданы", flush=True)
        return creds
    except json.JSONDecodeError as e:
        print(f"❌ Ошибка парсинга JSON: {e}", flush=True)
        raise
    except Exception as e:
        print(f"❌ Другая ошибка: {e}", flush=True)
        raise

def get_sheet(sheet_name):
    """Подключение к Google Sheets через переменные окружения"""
    print(f"📊 Подключаемся к листу {sheet_name}...", flush=True)
    creds = get_google_creds()
    client = gspread.authorize(creds)
    sheet = client.open("Бот_заявки_КА24")
    print(f"✅ Подключено к таблице, получаем лист {sheet_name}", flush=True)
...
def get_user_active_bookings(user_id):
    try:
        ...
        print(f"❌ Ошибка поиска записей: {e}", flush=True)
        return []
...
def cancel_booking(row_id):
    try:
        ...
        print(f"❌ Ошибка отмены записи: {e}", flush=True)
        return False
...
def handle_start(user_id, message):
    if message == 'записаться':
        ...
        if os.path.exists(PHOTO_PRICES):
            ...
            send_message(user_id, "💰 Прайс временно недоступен", keyboard=get_main_keyboard())
...
# ===== ОСНОВНОЙ ЦИКЛ =====
print("🚀 Инициализация VK сессии...", flush=True)
...
print("📌 Команды: Записаться, Отменить запись, Расписание, Прайс, Контакты, Отзыв, Админ", flush=True)
...
try:
    for event in longpoll.listen():
        if event.type == VkBotEventType.MESSAGE_NEW and event.from_user:
            ...
            print(f"📩 {user_id}: {message}", flush=True)
            ...
            if state == 'START':
                ...
                if message in ["✅ Да, все верно", "да", "yes", "+", "конечно", "подтверждаю"]:
                    ...
                    try:
                        ...
                        try:
                            ...
                            print(f"✅ Уведомление отправлено администратору", flush=True)
                        except Exception as e:
                            print(f"❌ Ошибка отправки админу: {e}", flush=True)
                        
                        user_states[user_id] = {'state': 'START', 'data': {}}
                    except Exception as e:
                        print(f"❌ Ошибка сохранения в Google: {e}", flush=True)
                ...
                if message in ["✅ Да, отменить", "да", "yes"]:
                    ...
                    if row_id and cancel_booking(row_id):
                        ...
                        try:
                            ...
                            vk.messages.send(user_id=ADMIN_VK_ID, message=admin_msg, random_id=random.randint(1, 10**9))
                            print(f"✅ Уведомление об отмене отправлено администратору", flush=True)
                        except Exception as e:
                            print(f"❌ Ошибка отправки админу: {e}", flush=True)
                    else:
                ...
                try:
                    ...
                    print(f"❌ Ошибка сохранения отзыва: {e}", flush=True)
                ...
                try:
                    ...
                    vk.messages.send(user_id=ADMIN_VK_ID, message=admin_msg, random_id=random.randint(1, 10**9))
                    print(f"✅ Вопрос переслан администратору", flush=True)
    ...
    # Держим бота живым, чтобы логи не закрылись
    import time
    while True:
        time.sleep(1)
